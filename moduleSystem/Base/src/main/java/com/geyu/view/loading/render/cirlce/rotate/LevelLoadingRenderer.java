package com.geyu.view.loading.render.cirlce.rotate;
...
import com.geyu.view.loading.render.LoadingRenderer;
...
public class LevelLoadingRenderer extends LoadingRenderer {
    ...
    protected void computeRender(float renderProgress) {
        ...
        if (renderProgress <= START_TRIM_DURATION_OFFSET) {
            ...
            mLevelSwipeDegrees[0] = MATERIAL_INTERPOLATOR.getInterpolation(startTrimProgress) * LEVEL_SWEEP_ANGLE_OFFSETS[0] *MAX_SWIPE_DEGREES ;
            mLevelSwipeDegrees[1] = MATERIAL_INTERPOLATOR.getInterpolation(startTrimProgress) * LEVEL_SWEEP_ANGLE_OFFSETS[1] *MAX_SWIPE_DEGREES ;
            mLevelSwipeDegrees[2] = MATERIAL_INTERPOLATOR.getInterpolation(startTrimProgress) * LEVEL_SWEEP_ANGLE_OFFSETS[2] *MAX_SWIPE_DEGREES ;
            mStartDegrees = mEndDegrees + mLevelSwipeDegrees[0];
        }

        if (renderProgress > START_TRIM_DURATION_OFFSET) {
            float endTrimProgress = (renderProgress - START_TRIM_DURATION_OFFSET) / (END_TRIM_DURATION_OFFSET - START_TRIM_DURATION_OFFSET);
            mLevelSwipeDegrees[0] = MATERIAL_INTERPOLATOR.getInterpolation(1f-endTrimProgress) * LEVEL_SWEEP_ANGLE_OFFSETS[0] *MAX_SWIPE_DEGREES ;
            mLevelSwipeDegrees[1] = MATERIAL_INTERPOLATOR.getInterpolation(1f-endTrimProgress) * LEVEL_SWEEP_ANGLE_OFFSETS[1] *MAX_SWIPE_DEGREES ;
            mLevelSwipeDegrees[2] = MATERIAL_INTERPOLATOR.getInterpolation(1f-endTrimProgress) * LEVEL_SWEEP_ANGLE_OFFSETS[2] *MAX_SWIPE_DEGREES ;
            mEndDegrees = mStartDegrees -  mLevelSwipeDegrees[0];
            mEndDegrees = mEndDegrees %360f;
        }
        mGroupRotation = renderProgress*360f;
    }
