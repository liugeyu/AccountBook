...
public class ShadowLayoutView extends FrameLayout {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (w > 4 && h > 4 ) { // 因为后面对宽高除了4 所以大于4而不能用大于0
    ...
    private void initAttributes(AttributeSet attrs) {
        TypedArray attr = getContext().obtainStyledAttributes(attrs, R.styleable.ShadowLayoutView);
        if (attr == null) {
            return;
        }
        ...
        try {
            //默认是显示
            leftShow = attr.getBoolean(R.styleable.ShadowLayoutView_shadow_leftShow, true);
            rightShow = attr.getBoolean(R.styleable.ShadowLayoutView_shadow_rightShow, true);
            bottomShow = attr.getBoolean(R.styleable.ShadowLayoutView_shadow_bottomShow, true);
            topShow = attr.getBoolean(R.styleable.ShadowLayoutView_shadow_topShow, true);
            mCornerRadius = attr.getDimension(R.styleable.ShadowLayoutView_shadow_cornerRadius, getResources().getDimension(R.dimen.dp_0));
            //默认扩散区域宽度
            mShadowLimit = attr.getDimension(R.styleable.ShadowLayoutView_shadow_shadowLimit, getResources().getDimension(R.dimen.dp_5));

            //x轴偏移量
            mDx = attr.getDimension(R.styleable.ShadowLayoutView_shadow_dx, 0);
            //y轴偏移量
            mDy = attr.getDimension(R.styleable.ShadowLayoutView_shadow_dy, 0);
            mShadowColor = attr.getColor(R.styleable.ShadowLayoutView_shadow_shadowColor, getResources().getColor(R.color.shadow_color));
            //判断传入的颜色值是否有透明度
            isAddAlpha(mShadowColor);
            mBackGroundColor = attr.getColor(R.styleable.ShadowLayoutView_shadow_shadowBackColor, getResources().getColor(R.color.white));
        } finally {
            attr.recycle();
        }
    }
